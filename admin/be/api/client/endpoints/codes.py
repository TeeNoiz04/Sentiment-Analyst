...
from models.schemas import CodeCreate, CodeUpdate, CodeResponse, CodeListResponse
...
async def create_code(code: CodeCreate, db: Session = Depends(get_db)):
    ...
    try:
        db_code = Code(
            Code=code.Code, CodeTypeId=code.CodeTypeId, Value=code.Value, Description=code.Description,
            Status=code.Status, CanDelete=code.CanDelete, CanEdit=code.CanEdit,
            DisplayAs=code.DisplayAs, PId=code.PId, ValueType=code.ValueType, Sequence=code.Sequence
        )
        db.add(db_code)
        db.commit()
        db.refresh(db_code)
...
async def get_codes(skip: int = Query(0, ge=0), limit: int = Query(10, ge=1, le=100), code_type_id: Optional[int] = None, status: Optional[str] = None, db: Session = Depends(get_db)):
    """Get list of codes"""
    query = db.query(Code)
    if code_type_id:
...
async def update_code(code_id: int, code_update: CodeUpdate, db: Session = Depends(get_db)):
    """Update a code"""
    code = db.query(Code).filter(Code.Id == code_id).first()
    if not code:
        raise HTTPException(status_code=404, detail="Code not found")
