...
from models.schemas import CodeTypeCreate, CodeTypeUpdate, CodeTypeResponse, CodeTypeListResponse
...
async def create_code_type(code_type: CodeTypeCreate, db: Session = Depends(get_db)):
    ...
    try:
        db_code_type = CodeType(CodeTypeName=code_type.CodeTypeName, Description=code_type.Description, IsActive=code_type.IsActive)
        db.add(db_code_type)
        db.commit()
...
async def get_code_types(skip: int = Query(0, ge=0), limit: int = Query(10, ge=1, le=100), is_active: Optional[bool] = None, db: Session = Depends(get_db)):
...
async def update_code_type(code_type_id: int, code_type_update: CodeTypeUpdate, db: Session = Depends(get_db)):
    """Update a code type"""
    code_type = db.query(CodeType).filter(CodeType.Id == code_type_id).first()
    if not code_type:
        raise HTTPException(status_code=404, detail="Code type not found")
