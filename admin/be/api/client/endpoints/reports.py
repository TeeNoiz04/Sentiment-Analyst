...
from models.schemas import ReportCreate, ReportUpdate, ReportResponse, ReportListResponse
...
async def create_report(report: ReportCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        existing_report = db.query(Report).filter(Report.PostID == report.PostID, Report.UserID == report.UserID).first()
        if existing_report:
            raise HTTPException(status_code=400, detail="User has already reported this post")
        
        db_report = Report(PostID=report.PostID, UserID=report.UserID, Reason=report.Reason, Status=ReportStatus.OPEN.value)
        db.add(db_report)
        db.commit()
